...
void Transform::ProjectPrepare(unsigned int columnID, float space,float hight)
{
    ...
    for (int i = 0; i < size(data[columnID]); ++i)
        data[columnID][i] = (data[columnID][i] - min_val) / (max_val - min_val) * (hight-2*space) - (hight-space);
}

    


std::vector<float> Transform::getColumn(unsigned int columnID) {
    ...
    for (int i = 0; i < size(data[columnID])/2+1; ++i) // /2 for half of the spectrum +1 one point more
        auxiliary.push_back( (float)std::abs(data[columnID][i]) );
    ...
    for (int i = 0; i < size(auxiliary); ++i)
        auxiliary[i] = ( auxiliary[i]-min_val ) / ( max_val-min_val ) *1.90 -0.95;
